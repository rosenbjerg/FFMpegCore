...
- Every ffmpeg flag is its own class in `FFMpegCore/FFMpeg/Arguments/` implementing `IArgument` (`Text` property), exposed through a method on `FFMpegInputOptions` (options that go before `-i`) and/or `FFMpegOutputOptions` (options that go before the output). The split is deliberate: `-c:v` before `-i` selects a decoder, `-c:v copy` there is an error, `-y` and `-map` make no sense on an input. Put a method on the side(s) where ffmpeg accepts the option; the nine shared ones (`-ss`, `-to`, `-t`, `-r`, `-f`, `-start_number`, `-threads`, custom, raw) are duplicated on purpose rather than pulled into a generic base. Adding a flag means: new `XxxArgument`, the method on the right options class(es), and an exact-string assertion in `ArgumentBuilderTest`.
