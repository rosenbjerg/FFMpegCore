...
- Every ffmpeg flag is its own class in `FFMpegCore/FFMpeg/Arguments/` implementing `IArgument` (`Text` property), exposed through a method on `FFMpegInputOptions` (options that go before `-i`) and/or `FFMpegOutputOptions` (options that go before the output). The split is deliberate: `-c:v` before `-i` selects a decoder, `-c:v copy` there is an error, `-y` and `-map` make no sense on an input. Put a method on the side(s) where ffmpeg accepts the option; the fifteen shared ones (`-ss`, `-to`, `-t`, `-r`, `-f`, `-pix_fmt`, `-ar`, `-start_number`, `-threads`, `-vn`, `-an`, `-sn`, `-dn`, custom, raw) are duplicated on purpose rather than pulled into a generic base. `-s` is the one deliberate exception: ffmpeg accepts it on an output, but `WithVideoFilters(f => f.Scale(…))` is the strictly more capable spelling there, so `WithFrameSize` is offered on the input side only. Adding a flag means: new `XxxArgument`, the method on the right options class(es), and an exact-string assertion in `ArgumentBuilderTest`.
...
- Global ffmpeg options are owned by the processor: `-v <loglevel>` from `FFOptions.LogLevel` / `WithLogLevel`, `-stats` when progress is requested. There is no user-facing global-options builder.
- Naming: an option method is named after the ffmpeg option it emits, using ffmpeg's vocabulary (`WithMap` for `-map`, `WithSpeedPreset` for `-preset`, `CopyStreams` for `-c copy`, `StreamType` for stream specifiers) unless ffmpeg's name hides what actually happens (`HardBurnSubtitle` for the `subtitles` filter). Each method's one-line `<summary>` is the ffmpeg option, so power users can search for `-ss` and land on `WithStartTime`.
...
`FFMpeg` (static) — `Snapshot`, `GifSnapshot`, `Join`, `SubVideo`, `Mute`, `ExtractAudio`, `ReplaceAudio`, `JoinImageSequence`, `PosterWithAudio`, `SaveM3U8Stream`, `Convert` — are thin compositions of the builder that return the `FFMpegArgumentProcessor` without running it; the caller picks `ProcessSynchronously`/`ProcessAsynchronously`. Each takes an optional `FFOptions` that covers the whole helper: the ffprobe call it makes before building the arguments, and the run itself, which it seeds onto the processor so a bare `ProcessSynchronously()` uses it. Don't probe inside a helper with `FFProbe.Analyse(input)` — a caller's `BinaryFolder` then applies to the run but not the probe. `Join` and `JoinImageSequence` end in `params`, so theirs is a separate overload taking `FFOptions` first, as `string.Format` does with its format provider. Anything a helper needs before the run (ffprobe for sizes and durations, extension checks) happens synchronously inside the helper; anything that must wrap the run lives in an argument's `Pre`/`Post` (`ImageSequenceInputArgument` copies the images into a temp folder and deletes it afterwards). `Join` is a single ffmpeg invocation through the `concat` filter, so its inputs must share a resolution. `SnapshotArgumentBuilder` is public so the image extensions can reuse the same argument construction and just swap the output for a `StreamPipeSink` + `ForceFormat("rawvideo")`.
